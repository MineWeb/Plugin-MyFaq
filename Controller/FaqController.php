...
class FaqController extends MyFaqAppController
{
    ...
    public function ajax_get_faq($id)
    {
        ...
        $faq = $this->Faq->find('first', ['conditions' => ['id' => $id],]);
        if (!empty($faq))
    ...
    public function admin_index()
    {
        if ($this->isConnected and $this->User->isAdmin()) {
            $this->set('title_for_layout', $this->Lang->get('FAQ'));
            $this->layout = 'admin';
            $faqs = $this->Faq->find('all', ['order' => 'id']);
            $this->set(compact("faqs"));
        } else
        ...
        if ($this->isConnected and $this->User->isAdmin()) {
            if ($this->request->is('post')) {
                ...
                if ($data['action'] == "edit") {
                    ...
                    $this->Faq->set(["question" => $data['question'], "answer" => $data['answer']]);
                    if ($faq = $this->Faq->save()) {
                        $return = json_encode(["action" => "add", "id" => $this->Faq->id, "question" => $data['question'], "answer" => $data['answer'],]);
                    } else
                        $return = 1;
                }
            } else
